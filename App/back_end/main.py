...
janela_mensagem = {'relx': height/2, 'rely': width/2, 'anchor': 'n'}
janela_entrada = {'relx': height/2, 'rely': width/2, 'anchor': 'n'}
janela_botao = {'relx': height/2, 'rely': width/2, 'anchor': 'n'}
...
def voltar():
    ...
    x_calculo = width/len(opcoes)

    for opcao in opcoes:
        botao = CTkButton(app, text=opcao, width=width/len(opcoes), height=120, command=lambda opcao=opcao: selecionar_opcao(opcao), corner_radius=32, hover=True, border_color="", font=("Arial", 20), bg_color="#242424", fg_color="black")
        botao.place(x=x_position, y=y_position)

        x_position += x_calculo
    text_central = tk.Label(app, text="Gerenciador de Senhas", bg="#242424", fg="white", font=("Helvetica", 16))
...
def render():
    ...
    for opcao in opcoes:
        button = CTkButton(app, text=opcao, )
...
label = CTkLabel(app, text="Tela Cheia", font=("Helvetica", 24))
